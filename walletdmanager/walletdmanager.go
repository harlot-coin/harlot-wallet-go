...
import (
	"Harlot-Nest/turtlecoinwalletdrpcgo"
	"bufio"
	"io"
	"math/rand"
	"os"
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "SEXG") || (len(transferAddress) != 99 && len(transferAddress) != 187) {
	...
	if transferAmount <= 0 {
		return "", errors.New("amount of HRLT to be sent should be greater than 0")
	}
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isPlatformDarwin {
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/Harlot-Nest"
	...
	if err != nil {
		killWalletd()
		return errors.New("error communicating with harlot-service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill harlot-service: " + err.Error())
			} else {
				log.Info("harlot-service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill harlot-service: " + err.Error())
				}
				log.Info("harlot-service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("harlot-service finished with error: " + err.Error())
				}
				log.Info("harlot-service killed without error")
			}
		}
...
func GracefullyQuitTurtleCoind() {
	...
	if cmdTurtleCoind != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill Harlotd: " + err.Error())
			} else {
				log.Info("Harlotd killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdTurtleCoind.Process.Kill(); err != nil {
					log.Warning("failed to kill Harlotd: " + err.Error())
				}
				log.Info("Harlotd killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("Harlotd finished with error: " + err.Error())
				}
				log.Info("Harlotd killed without error")
			}
		}
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("harlot-service is already running. It should be stopped before being able to generate a new wallet")
	}
	...
	if isWalletdRunning() {
		errorMessage := "harlot-service is already running in the background.\nPlease close it via "
		...
		for {
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "harlot-service stopped with unknown error"
				}
